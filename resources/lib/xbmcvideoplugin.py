...
class XBMCVideoPlugin(object):
    ...
    def play_video(self, url, info=None, properties=None):
        ...
        if properties: 
            [item.setProperty(k, v) for k, v in properties.items()]
        xbmc.Player(xbmc.PLAYER_CORE_AUTO).play(url, item)
        #xbmcplugin.setResolvedUrl(self.argv1, True, item)
    ...
    def add_videos(self, lis, end=True):
        ...
        _lis = [self.make_directory_item(li, isfolder=False, make_plugin_url=False) for li in lis]
    ...
    def run(self):
        ...
        current_handler = handler(self.argv0, self.argv1, self, self.params)
        try:
            current_handler.run()
